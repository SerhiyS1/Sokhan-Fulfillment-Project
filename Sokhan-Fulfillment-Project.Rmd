...
xy <- monthandcountry %>%
  ...
  filter(billing_country == "United States") %>%
  select(month, billing_country, orders, num_orders, average_order_value) 
  

ggplot(data = xy, aes(x = month, y = num_orders)) + stat_summary(geom = "bar")
...
```
big shoutout https://stackoverflow.com/questions/11857935/plotting-the-average-values-for-each-level-in-ggplot2

used to write out every country until I got choices from https://stackoverflow.com/questions/42957746/shiny-create-reactive-filter-using-different-variables

link to shinythemes package
https://rstudio.github.io/shinythemes/

link to thing helping me w one inputID
https://stackoverflow.com/questions/50852305/one-reactive-function-to-be-displayed-on-two-different-pages-interactively

link to hyperlink to github
